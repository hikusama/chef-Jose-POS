...
=======
    ...
    public function topProd()
    {
        $sql = "SELECT
                    ...
                    FROM orders AS ord
                    LEFT JOIN orderitems AS oi ON oi.ref_no = ord.ref_no 
                    LEFT JOIN products AS prd ON prd.productID = oi.productID
                    WHERE YEARWEEK(ord.orderDate, 1) = YEARWEEK(CURRENT_DATE(), 1)
...
>>>>>>> dockerized
    ...
    public function pdfDataMain($range)
    {
        $sql = "SELECT ";
        if (count($range) == 1) {
            $sql .= "
                SUM(CASE WHEN DATE(orderDate) = :starting THEN totalAmount ELSE 0 END) AS sales,
                SUM(CASE WHEN DATE(orderDate) = :starting THEN 1 ELSE 0 END) AS ordered,
                SUM(CASE WHEN DATE(orderDate) = :starting THEN discount ELSE 0 END) AS discounts";
        } else {
            $sql .= "
                SUM(CASE WHEN DATE(orderDate) BETWEEN :starting AND :ending THEN totalAmount ELSE 0 END) AS sales,
                SUM(CASE WHEN DATE(orderDate) BETWEEN :starting AND :ending THEN 1 ELSE 0 END) AS ordered,
                SUM(CASE WHEN DATE(orderDate) BETWEEN :starting AND :ending THEN discount ELSE 0 END) AS discounts";
        }
        $sql .= " FROM orders";
        ...
        if (count($range) == 1) {
            $stmt->bindParam(':starting', $range[0], PDO::PARAM_STR);
        } else {
            $stmt->bindParam(':starting', $range[0], PDO::PARAM_STR);
            $stmt->bindParam(':ending', $range[1], PDO::PARAM_STR);
        }
    ...
    public function pdfOrderList($range)
    {
        $sql = "SELECT 
                ord.*,
				ad.name AS dname,
				e.fName AS ename,
                COUNT(DISTINCT oi.orderItemId) AS totalItems
                FROM orders AS ord 
                LEFT JOIN orderitems AS oi ON oi.ref_no = ord.ref_no
                LEFT JOIN employees AS e ON e.userID = ord.userID
                LEFT JOIN admin AS ad ON ad.userID = ord.userID WHERE
        ";

        if (count($range) == 1) {
            $sql .= " DATE(orderDate) = :starting";
        } else {
            $sql .= " DATE(orderDate) BETWEEN :starting AND :ending";
        }
    ...
    public function pdfTopProd($range)
    {
        $sql = "SELECT
                    prd.name,
                    prd.productID,
                    SUM(oi.unitPrice) as sales,
                    SUM(oi.quantity) as total_order
                    FROM orders AS ord
                    LEFT JOIN orderitems AS oi ON oi.ref_no = ord.ref_no 
                    LEFT JOIN products AS prd ON prd.productID = oi.productID WHERE ";
        if (count($range) == 1) {
            $sql .= " DATE(orderDate) = :starting";
        } else {
            $sql .= " DATE(orderDate) BETWEEN :starting AND :ending";
        }
        $sql .= "  AND prd.name IS NOT NULL GROUP BY prd.productID
                    ORDER BY total_order DESC LIMIT 10 OFFSET 0;";
    ...
    public function pdfTopCombo($range)
    {
        $sql = "SELECT
                    cmb.comboName,
                    cmb.comboID,
                    ...
                    FROM orders AS ord
                    LEFT JOIN orderitems AS oi ON oi.ref_no = ord.ref_no 
                    LEFT JOIN combo AS cmb ON cmb.comboID = oi.comboID WHERE ";
        if (count($range) == 1) {
            $sql .= "DATE(orderDate) = :starting";
        } else {
            $sql .= "DATE(orderDate) BETWEEN :starting AND :ending";
        }
        $sql .= " AND cmb.comboName IS NOT NULL GROUP BY cmb.comboID
                ORDER BY total_order DESC LIMIT 10 OFFSET 0;";
        $stmt = $this->connect()->prepare($sql);
    ...
    public function pdfLeastProd($range)
    {
        ...
        $sql = "SELECT
                ...
                WHERE prd.productID NOT IN (
                    SELECT oi.productID FROM orders AS ord
                    LEFT JOIN orderitems AS oi ON oi.ref_no = ord.ref_no WHERE ";
        if (count($range) == 1) {
            $sql .= "DATE(orderDate) = :starting";
        } else {
            $sql .= "DATE(orderDate) BETWEEN :starting AND :ending";
        }
        $sql .= " AND oi.productID IS NOT NULL) GROUP BY prd.productID;";
    ...
    public function pdfLeastCombo($range)
    {
        $sql = "SELECT
                    cmb.comboName,
                    cmb.comboID
                    FROM combo AS cmb
                    LEFT JOIN orderitems AS oi ON oi.comboID = cmb.comboID 
                    WHERE cmb.comboID NOT IN (
                        SELECT oi.comboID FROM orders AS ord
                        LEFT JOIN orderitems AS oi ON oi.ref_no = ord.ref_no WHERE ";
        if (count($range) == 1) {
            $sql .= "DATE(orderDate) = :starting";
        } else {
        ...
        $sql .= " AND oi.comboID IS NOT NULL) GROUP BY cmb.comboID;";
        $stmt = $this->connect()->prepare($sql);
